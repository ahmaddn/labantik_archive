...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-3">
            @if (!$isConnected)
                <span
                    class="inline-flex items-center gap-1.5 rounded-xl border border-yellow-200 bg-yellow-50 px-3 py-1.5 text-xs font-medium text-yellow-700">
    ...
    @if ($quotaMb)
        <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div class="col-span-1 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm sm:col-span-3">
                <div class="mb-3 flex items-center justify-between">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm font-semibold text-gray-700">Penggunaan</span>
                    </div>
                    <span class="text-sm font-bold text-gray-800">{{ $usedMb }} / {{ $quotaMb }} MB <span
                            class="font-normal text-gray-400">({{ $usedPct }}%)</span></span>
                </div>
                ...
                <p class="mt-2 text-xs text-gray-400">Sisa: <strong class="text-gray-600">{{ $remainingMb }} MB</strong>
                </p>
            </div>
        </div>
        <div class="mb-6 flex items-center gap-3">
            @if (auth()->user()->isSiswa())
                @include('drive.components.signature-modal')
            @endif
        </div>
    @endif
                ...
                <div class="{{ $c['border'] }} overflow-hidden rounded-2xl border shadow-sm">
                    ...
                    <button type="button" onclick="toggleCat('{{ $boxId }}')"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span
                                class="text-sm font-bold tracking-wide text-white">{{ $categoryData['category']->name }}</span>
                        ...
                        @else
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm">
                                    <thead
                                        class="{{ $c['border'] }} border-b text-xs uppercase tracking-wider text-gray-500">
                                        <tr>
                                        ...
                                        @foreach ($categoryData['files'] as $file)
                                            <tr class="transition-colors hover:bg-white/60">
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center gap-3">
                                                        <div
                                                            class="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-white shadow-sm">
                                                        ...
                                                        <div>
                                                            <span
                                                                class="block font-semibold leading-tight text-gray-800">{{ $file->document_name }}</span>
                                                ...
                                                <td class="hidden px-6 py-4 text-gray-500 xl:table-cell">
                                                    @if ($file->expertise_label !== '-')
                                                        <span
                                                            class="{{ $c['badge'] }} rounded-full px-2 py-0.5 text-xs font-medium">
                                                ...
                                                <td class="hidden px-6 py-4 text-xs text-gray-500 sm:table-cell">
                                                ...
                                                <td class="px-6 py-4">
                                                    <div class="flex items-center justify-center gap-2">
                                                        ...
                                                        <form method="POST"
                                                            ...
                                                            @method('DELETE')
                                                            <button type="submit"
                                                                class="flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-medium text-red-500 shadow-sm transition-colors hover:bg-red-50">
    ...
    @elseif(isset($files) && $files->count() > 0)
        <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 text-xs uppercase tracking-wider text-gray-500">
                        <tr>
                            ...
                            <th class="hidden px-6 py-3 text-left lg:table-cell">Kategori</th>
                            <th class="hidden px-6 py-3 text-left xl:table-cell">Keahlian</th>
                            <th class="hidden px-6 py-3 text-left md:table-cell">Ukuran</th>
                            <th class="hidden px-6 py-3 text-left sm:table-cell">Tanggal</th>
                            ...
                            <tr class="transition-colors hover:bg-gray-50">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <span
                                                class="block font-semibold text-gray-800">{{ $file->document_name }}</span>
                                            <span class="text-xs text-gray-400">{{ $file->name }}</span>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="hidden px-6 py-4 text-gray-500 md:table-cell">{{ $file->formatted_size }}</td>
                                <td class="hidden px-6 py-4 text-gray-500 sm:table-cell">
                                    {{ $file->created_at->format('d M Y') }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ $file->web_content_link }}" download
                                            class="flex items-center gap-1.5 rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-medium text-blue-700 hover:bg-blue-100">
                                            Unduh
                                        </a>
                                        <a href="{{ $file->web_view_link }}" target="_blank"
                                            class="flex items-center gap-1.5 rounded-lg bg-green-50 px-3 py-1.5 text-xs font-medium text-green-700 hover:bg-green-100">
                                            Lihat
                                        </a>
                                        <form method="POST" action="{{ route('drive.destroy', $file->id) }}"
                                            ...
                                            @csrf @method('DELETE')
                                            <button
                                                class="flex items-center gap-1.5 rounded-lg bg-red-50 px-3 py-1.5 text-xs font-medium text-red-700 hover:bg-red-100">
                                                Hapus
                                            </button>
                                        </form>
                                    </div>
            ...
            @if ($files->hasPages())
                <div class="border-t border-gray-100 px-6 py-4">{{ $files->links() }}</div>
            @endif
        </div>
    @else
        {{-- Empty state --}}
        <div class="rounded-2xl border border-gray-200 bg-white py-20 text-center shadow-sm">
            ...
            <p class="text-sm font-medium text-gray-500">Belum ada dokumen. Tambahkan dokumen pertama Anda!</p>
            <a href="{{ route('drive.create') }}"
                class="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#1b84ff] px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-[#1570e0]">
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                </svg>
                Tambah Dokumen
