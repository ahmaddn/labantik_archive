...
@section('content')
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Rekap seluruh dokumen yang diunggah oleh pengguna.</p>
        </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div class="flex items-center gap-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div>
                ...
                <p class="text-xs font-medium text-gray-500">Pengguna yang Upload</p>
            </div>
        </div>
        <div class="flex items-center gap-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div>
                ...
                <p class="text-xs font-medium text-gray-500">Total Ukuran</p>
            </div>
        </div>
    </div>
    ...
    <div class="mb-4 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        <form method="GET" action="{{ route('admin.history.index') }}" class="flex flex-col gap-3 sm:flex-row">
            {{-- Search --}}
            ...
            <select name="per_page"
                ...
                @foreach ([10, 15, 25, 50, 100] as $opt)
                    <option value="{{ $opt }}" {{ $perPage == $opt ? 'selected' : '' }}>{{ $opt }} per
                        hal.</option>
                @endforeach
            </select>

            <button type="submit"
                class="inline-flex flex-shrink-0 items-center gap-2 rounded-xl bg-[#1b84ff] px-5 py-2 text-sm font-semibold text-white shadow-sm shadow-blue-200 transition-colors hover:bg-[#1570e0]">
            ...
            @if ($search || $roleFilter)
                <a href="{{ route('admin.history.index') }}"
                    class="inline-flex flex-shrink-0 items-center gap-2 rounded-xl bg-gray-100 px-4 py-2 text-sm font-medium text-gray-600 transition-colors hover:bg-gray-200">
                    Reset
                </a>
            @endif
        </form>
    ...
    <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        ...
        @if ($files->isEmpty())
            <div class="py-20 text-center text-gray-400">
                ...
                <p class="text-sm font-medium">Belum ada dokumen yang
                    diunggah{{ $search ? ' dengan kata kunci tersebut' : '' }}.</p>
            </div>
        @else
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 text-xs uppercase tracking-wider text-gray-500">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left font-semibold">Tanggal Upload</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @foreach ($files as $i => $file)
                            ...
                            <tr class="transition-colors hover:bg-gray-50">
                                <td class="px-6 py-4 text-xs font-medium text-gray-400">{{ $rowNum }}</td>

                                {{-- Pengguna --}}
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-[#1b84ff] to-[#0ea5e9] text-xs font-bold text-white">
                                            {{ strtoupper(substr($user?->name ?? '?', 0, 2)) }}
                                        </div>
                                        <div>
                                            <span class="block font-semibold leading-tight text-gray-800">
                                                {{ $user?->name ?? '—' }}
                                            </span>
                                            <div class="mt-0.5 flex items-center gap-1.5">
                                                ...
                                                @if ($badge)
                                                    <span
                                                        class="{{ $badge['bg'] }} {{ $badge['text'] }} inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-semibold">
                                                        {{ $badge['label'] }}
                                                    </span>
                                                @endif
                                            </div>
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2.5">
                                        <div
                                            class="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-gray-100">
                                            @if (str_contains($file->mime_type ?? '', 'pdf'))
                                                <svg class="h-3.5 w-3.5 text-red-500" fill="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path
                                                        d="M14 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z" />
                                                </svg>
                                            @else
                                                <svg class="h-3.5 w-3.5 text-blue-400" fill="currentColor"
                                                    viewBox="0 0 24 24">
                                                    <path
                                        ...
                                        <div>
                                            <span
                                                class="block text-xs font-semibold leading-tight text-gray-800">{{ $file->document_name }}</span>
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @else
                                        <span class="text-xs italic text-gray-400">Tanpa Kategori</span>
                                    @endif
                                </td>
            ...
            @if ($files->hasPages())
                <div
                    ...
                    <p class="order-2 text-xs text-gray-400 sm:order-1">
